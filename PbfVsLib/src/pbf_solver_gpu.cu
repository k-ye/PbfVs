...
__device__ bool IsInside(const float3 &pt, const float3 &min,
                         const float3 &max) {
  bool cond = (min.x <= pt.x) && (pt.x <= max.x) && (min.y <= pt.y) &&
              (pt.y <= max.y) && (min.z <= pt.z) && (pt.z <= max.z);
  return cond;
}
...
__global__ void CellGridEntryPointKernel(const float3 *positions,
                                         const int num_ptcs,
                                         const float cell_sz,
                                         const int3 num_cells_dim,
                                         int *ptc_to_cell, int *cell_num_ptcs,
                                         int *ptc_offsets_within_cell) {
...
void ComputeCellToActiveCellIndices(
  ...
  assert(cell_is_active_flags.size() == cell_to_active_cell_indices->size());
  thrust::exclusive_scan(thrust::device, cell_is_active_flags.begin(),
                         cell_is_active_flags.end(),
                         cell_to_active_cell_indices->begin(), 0);
}

__global__ void CompactKernel(const int *input, const int *flag,
                              const int *compact_indices, const int size,
                              int *output) {
...
void ComputeActiveCellNumPtcs(const d_vector<int> &cell_num_ptcs,
  ...
  const int *compact_indices =
      thrust::raw_pointer_cast(cell_to_active_cell_indices.data());
...
void ComputePtcBeginsInActiveCell(const d_vector<int> &active_cell_num_ptcs,
                                  d_vector<int> *ptc_begins_in_active_cell) {
  assert(active_cell_num_ptcs.size() == ptc_begins_in_active_cell->size());
  thrust::exclusive_scan(thrust::device, active_cell_num_ptcs.begin(),
                         active_cell_num_ptcs.end(),
...
__global__ void ComputeCellPtcIndicesKernel(
    ...
    const int *ptc_begins_in_active_cell, const int *ptc_offsets_within_cell,
...
__global__ void CountPtcNumNeighborsKernel(
    const float3 *positions, const int *cell_is_active_flags,
    ...
    for (int cy = -1; cy <= 1; ++cy) {
      for (int cx = -1; cx <= 1; ++cx) {
        ...
        if (!cell_is_active_flags[nb_cell_idx])
          continue;
        const int nb_ac_idx = cell_to_active_cell_indices[nb_cell_idx];
...
void ComputePtcNeighborBegins(const d_vector<int> &ptc_num_neighbors,
  ...
  assert(ptc_num_neighbors.size() == ptc_neighbor_begins->size());
  thrust::exclusive_scan(thrust::device, ptc_num_neighbors.begin(),
                         ptc_num_neighbors.end(), ptc_neighbor_begins->begin(),
                         0);
}
...
__global__ void FindPtcNeighborIndicesKernel(
    ...
    const int *ptc_begins_in_active_cell, const int *active_cell_num_ptcs,
    const int num_ptcs, const float cell_sz, const int3 num_cells_dim,
    const float radius, const int *ptc_neighbor_begins,
    int *ptc_neighbor_indices,
...
__global__ static void QueryCountKernel(
    const int num_cells, const float3 range_min, const float3 range_max,
    const float3 *positions, const int *cell_is_active_flags,
    const int *cell_to_active_cell_indices,
    const int *ptc_begins_in_active_cell, const int *active_cell_num_ptcs,
...
CellGridGpu::CellGridGpu(float3 world_sz, float cell_sz)
  ...
  total_num_cells_ =
      num_cells_per_dim_.x * num_cells_per_dim_.y * num_cells_per_dim_.z;
}
...
void UpdateCellGrid(const d_vector<float3> &positions, CellGridGpu *cell_grid) {
  ...
  d_vector<int> &ptc_offsets_within_cell = cell_grid->ptc_offsets_within_cell;
  ...
  CellGridEntryPointKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      ...
      cell_grid->num_cells_per_dim(), ptc_to_cell_ptr, cell_num_ptcs_ptr,
      ptc_offsets_within_cell_ptr);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
  ...
  int *cell_is_active_flags_ptr = raw_pointer_cast(cell_is_active_flags.data());
  ...
  ComputePtcBeginsInActiveCell(active_cell_num_ptcs,
                               &ptc_begins_in_active_cell);
...
void FindParticleNeighbors(const d_vector<float3> &positions,
  ...
  CountPtcNumNeighborsKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      positions_ptr, cell_is_active_flags_ptr, cell_to_active_cell_indices_ptr,
      cell_ptc_indices_ptr, ptc_begins_in_active_cell_ptr,
      active_cell_num_ptcs_ptr, num_ptcs, cell_sz, num_cells_dim, h,
      ptc_num_neighbors_ptr);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
  ...
  const int *ptc_neighbor_begins_ptr =
      raw_pointer_cast(ptc_neighbor_begins.data());

  // make sure we allocate memory first
  d_vector<int> &ptc_neighbor_indices = pn->ptc_neighbor_indices;
  {
    int vec_sz =
        thrust::reduce(ptc_num_neighbors.begin(), ptc_num_neighbors.end(), 0);
  ...
  FindPtcNeighborIndicesKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      positions_ptr, cell_is_active_flags_ptr, cell_to_active_cell_indices_ptr,
      cell_ptc_indices_ptr, ptc_begins_in_active_cell_ptr,
      active_cell_num_ptcs_ptr, num_ptcs, cell_sz, num_cells_dim, h,
      ptc_neighbor_begins_ptr, ptc_neighbor_indices_ptr,
      ptc_num_neighbors_ptr /*debug purpose, rm once correct*/);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}
...
void Query(const d_vector<float3> &positions, const CellGridGpu &cell_grid,
  ...
  QueryCountKernel<<<num_blocks_cell, kNumThreadPerBlock>>>(
      num_cells, range_min, range_max, positions_ptr, cell_is_active_flags_ptr,
      cell_to_active_cell_indices_ptr, ptc_begins_in_active_cell_ptr,
      active_cell_num_ptcs_ptr, cell_ptc_indices_ptr, cell_num_ptcs_inside_ptr);
}
...
ComputeLambdaKernel(const float3 *positions, const int *ptc_num_neighbors,
                    const int *ptc_neighbor_begins,
                    const int *ptc_neighbor_indices, const int num_ptcs,
                    const float h, const float mass, const float rho_0_recpr,
                    const float epsilon, float *lambdas) {
  const int ptc_i = (blockDim.x * blockIdx.x) + threadIdx.x;
  if (ptc_i >= num_ptcs)
    return;
...
__device__ float ComputeScorr(const float3 pos_ji, const float h,
                              const float corr_delta_q_coeff,
                              const float corr_k, const float corr_n) {
  // Eq (13)
...
__global__ static void ComputeDeltaPositionsKernel(
    const float3 *positions, const int *ptc_num_neighbors,
    const int *ptc_neighbor_begins, const int *ptc_neighbor_indices,
    const float *lambdas, const int num_ptcs, const float h,
    const float rho_0_recpr, const float corr_delta_q_coeff, const float corr_k,
    const float corr_n, float3 *delta_positions) {
  const int ptc_i = (blockDim.x * blockIdx.x) + threadIdx.x;
  if (ptc_i >= num_ptcs)
    return;
  ...
  for (int offs = 0; offs < num_nbs; ++offs) {
    ...
    const float scorr_ij =
        ComputeScorr(pos_ji, h, corr_delta_q_coeff, corr_k, corr_n);
...
__global__ static void
UpdateVelocitiesKernel(const float3 *old_positions, const float3 *new_positions,
...
__global__ static void
ComputeVorticitiesKernel(const float3 *positions, const float3 *velocities,
                         const int *ptc_num_neighbors,
                         const int *ptc_neighbor_begins,
                         const int *ptc_neighbor_indices, const int num_ptcs,
                         const float h, float3 *vorticities) {
  const int ptc_i = (blockDim.x * blockIdx.x) + threadIdx.x;
  if (ptc_i >= num_ptcs)
    return;
...
__global__ static void ComputeVorticityCorrForcesKernel(
    const float3 *positions, const int *ptc_num_neighbors,
    const int *ptc_neighbor_begins, const int *ptc_neighbor_indices,
    const float3 *vorticities, const int num_ptcs, const float h,
    const float vorticity_epsilon, float3 *vorticity_corr_forces) {
  const int ptc_i = (blockDim.x * blockIdx.x) + threadIdx.x;
  if (ptc_i >= num_ptcs)
    return;
...
__global__ static void
ComputeXsphsKernel(const float3 *positions, const float3 *velocities,
                   const int *ptc_num_neighbors, const int *ptc_neighbor_begins,
                   const int *ptc_neighbor_indices, const int num_ptcs,
                   const float h, const float xsph_c, float3 *xsphs) {
  const int ptc_i = (blockDim.x * blockIdx.x) + threadIdx.x;
  if (ptc_i >= num_ptcs)
    return;
...
__global__ static void
ApplyVelocityCorrectionsKernel(const float3 *vorticity_corr_forces,
                               const float3 *xsphs, const int num_ptcs,
                               const float dt, float3 *velocities) {
  const int ptc_i = (blockDim.x * blockIdx.x) + threadIdx.x;
  if (ptc_i >= num_ptcs)
    return;
...
void PbfSolverGpu::ResetParticleRecords_() {
  ...
  fill(device, vorticity_corr_forces_.begin(), vorticity_corr_forces_.end(),
       zeros);
...
void PbfSolverGpu::RecordOldPositions_() {
  ...
  thrust::copy(thrust::device, d_positions->begin(), d_positions->end(),
               old_positions_.begin());
}

void PbfSolverGpu::ApplyGravity_(const float dt) {
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);
  ApplyGravityKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      num_ptcs_, dt, PositionsPtr_(), VelocitiesPtr_());
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}
...
void PbfSolverGpu::FindNeighbors_() {
  const float3 world_sz_dim =
      make_float3(world_size_x_, world_size_y_, world_size_z_);
...
void PbfSolverGpu::ComputeLambdas_() {
  ...
  ComputeLambdaKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      PositionsPtr_(), ptc_nb_recs_.ptc_num_neighbors_ptr(),
      ptc_nb_recs_.ptc_neighbor_begins_ptr(),
      ptc_nb_recs_.ptc_neighbor_indices_ptr(), num_ptcs_, h_, mass_,
      rho_0_recpr_, epsilon_, lambdas_ptr);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}
...
void PbfSolverGpu::ComputeDeltaPositions_() {
  ...
  float3 *delta_positions_ptr =
      thrust::raw_pointer_cast(delta_positions_.data());
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);
  ComputeDeltaPositionsKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      PositionsPtr_(), ptc_nb_recs_.ptc_num_neighbors_ptr(),
      ptc_nb_recs_.ptc_neighbor_begins_ptr(),
      ptc_nb_recs_.ptc_neighbor_indices_ptr(), lambdas_ptr, num_ptcs_, h_,
      rho_0_recpr_, corr_delta_q_coeff_, corr_k_, corr_n_, delta_positions_ptr);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}

void PbfSolverGpu::ApplyDeltaPositions_() {
  const float3 *delta_positions_ptr =
      thrust::raw_pointer_cast(delta_positions_.data());
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);
...
void PbfSolverGpu::UpdateVelocities_(const float dt) {
  const float3 *old_positions_ptr =
      thrust::raw_pointer_cast(old_positions_.data());
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);
  UpdateVelocitiesKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      old_positions_ptr, PositionsPtr_(), num_ptcs_, dt, VelocitiesPtr_());
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}
...
void PbfSolverGpu::ComputeVorticities_() {
  ...
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);
  ComputeVorticitiesKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      PositionsPtr_(), VelocitiesPtr_(), ptc_nb_recs_.ptc_num_neighbors_ptr(),
      ptc_nb_recs_.ptc_neighbor_begins_ptr(),
...
void PbfSolverGpu::ComputeVorticityCorrForces_() {
  ...
  float3 *vort_corr_forces_ptr =
      thrust::raw_pointer_cast(vorticity_corr_forces_.data());
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);

  ComputeVorticityCorrForcesKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      PositionsPtr_(), ptc_nb_recs_.ptc_num_neighbors_ptr(),
      ptc_nb_recs_.ptc_neighbor_begins_ptr(),
      ptc_nb_recs_.ptc_neighbor_indices_ptr(), vorticities_ptr, num_ptcs_, h_,
      vorticity_epsilon_, vort_corr_forces_ptr);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}
...
void PbfSolverGpu::ComputeXsphs_() {
  ...
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);
  ComputeXsphsKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      PositionsPtr_(), VelocitiesPtr_(), ptc_nb_recs_.ptc_num_neighbors_ptr(),
      ptc_nb_recs_.ptc_neighbor_begins_ptr(),
      ptc_nb_recs_.ptc_neighbor_indices_ptr(), num_ptcs_, h_, xsph_c_,
      xsphs_ptr);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}
...
void PbfSolverGpu::ApplyVelocityCorrections_(const float dt) {
  const float3 *vort_corr_forces_ptr =
      thrust::raw_pointer_cast(vorticity_corr_forces_.data());
  const float3 *xsphs_ptr = thrust::raw_pointer_cast(xsphs_.data());
  const int num_blocks_ptc = impl_::ComputeNumBlocks(num_ptcs_);
  ApplyVelocityCorrectionsKernel<<<num_blocks_ptc, kNumThreadPerBlock>>>(
      vort_corr_forces_ptr, xsphs_ptr, num_ptcs_, dt, VelocitiesPtr_());
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
}
