...
namespace math_impl_{
	...
	struct Tagger {
		typedef typename IfElse<
			...
			FloatingPointTag,
			// Cannot use std::enable_if here, because type system is a compile time thing.
			// That is, all types must be known at compile type, there is no short circuit.
			// This results in the definition of UnknownTag and is not so fancy.
			typename IfElse<
